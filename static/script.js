const API_URL = '/yeni-olay-cifti';
 ...
 function ekranaYerlestir() {
     ...
     cardsContainer.classList.remove('is-hidden'); // Kartları görünür yap (fade in)
     zamanlayiciyiBaslat();
 }
 ...
 function tahminEt(tahmin) {
     ...
     if (tahmin === gercekDurum) {
         ...
         setTimeout(() => {
             cardsContainer.classList.add('is-hidden'); // Kartları gizle (fade out)

             sabitOlay = tahmin === "ayni" ? null : olay2;
             kullanilmis_idler.push(olay2.id);

             // Kartlar gizlenirken yeni veriyi getir
